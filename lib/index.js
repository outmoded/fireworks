...
exports.neopixels = function (animation) {
    ...
    for (var g = 0, gl = segments.length; g < gl; ++g) {
        ...
        for (var s = 0; s < 3; ++s) {
            ...
            for (var i = Pos.mark[segment][0], il = Pos.mark[segment][1]; i < il; ++i) {
                var frame = strand[i];
                var neoFrame = new Buffer(frame.length * 3);
                for (var p = 0, pl = frame.length; p < pl; ++p) {
                    var color = frame[p];
