...
internals.color = function (color, pos, size) {
    ...
    if (!Array.isArray(color)) {
        return !size ? color : internals.fade(color, size - pos, size);
    }
...
internals.whirl = function (color, size) {
    ...
    var path = [0, 0, 2, 1, 1, 2]; // [0, 0, 1, 2, 2, 1];

    var sequences = [[], [], []];
    for (var i = 0, il = height - size; i < il; ++i) {
    ...
    var draw = function (color) {
        ...
        for (var i = 0; i < size; ++i) {
            ...
            for (var l = 0; l < 4; ++l) {
                ...
                frames[strand][led + (i * direction)] = internals.color(color, i, size);
            }
