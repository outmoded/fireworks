...
internals.color = function (color, pos) {
...
internals.launch = function (color, size, blanks) {
    ...
    for (var i = start; i <= end; ++i) {
        ...
        for (var s = 0; s < size && s + i < end; ++s) {
            frame[i + s] = internals.color(color, s);
        }
...
internals.burst = function (color, size) {
    ...
    for (var i = start; i <= end; ++i) {
        ...
        for (var s = 0; s < size && s + i < end; ++s) {
            frame[i + s] = internals.color(color, s);
            frame[i + s + offset] = internals.color(color, s);
        }
...
internals.curve = function (color, duration) {
    ...
    for (var i = 0; i < duration; ++i) {
        ...
        for (var s = start; s <= end; ++s) {
            frame[s] = Math.random() > 0.9 ? internals.color(color, 0) : 0;
        }
    ...
    var draw = function (color) {
        ...
        for (var i = 0; i < size; ++i) {
            ...
            for (var l = 0; l < 4; ++l) {
                ...
                frames[strand][led + (i * direction)] = internals.color(color, i);
            }
