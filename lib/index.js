...
internals.render = function (element) {
    ...
    if (element.type === 'overlay') {
        var first = Array.isArray(element.first) ? exports.compile(element.first) : internals.render(element.first);
        var second = Array.isArray(element.second) ? exports.compile(element.second) : internals.render(element.second);
    ...
    if (element.type === 'launch') {
        for (i = 0, il = 3; i < il; ++i) {
            segment[i] = internals.launch(colors[i], element.sizes[i]);
        }
    }

    if (element.type === 'burst') {
        for (i = 0, il = 3; i < il; ++i) {
            segment[i] = internals.burst(colors[i], element.sizes[i]);
        }
    }

    if (element.type === 'tails') {
        for (i = 0, il = 3; i < il; ++i) {
            segment[i] = internals.tails(colors[i], element.sizes[i]);
        }
    }
    ...
    if (element.type === 'stars') {
        ...
        segment = internals.star(colors[0], element.size, element.location, 0);

        for (i = 1; i < count; ++i) {
            var seq = internals.star(colors[i], element.size, element.location, i);
            for (var s = 0; s < 3; ++s) {
